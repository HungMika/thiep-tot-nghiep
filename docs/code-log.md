...
18/07/2026: route refactor — deleted src/app/invite/[slug], added src/app/[hostName] (page + loading), created features/invite/utils/fetchSheet.ts (TSV fetch + parse), replaced hardcoded EVENT with EventConfig type in constants.ts, updated InviteCard/InviteCardExport/CountdownTimer/MapButton/InvitePageView to accept EventConfig as props, updated useCountdown to accept target:Date param, updated HomeForm+useHomeForm to collect hostName and redirect to /{hostName}, created src/components/NotFound.tsx (Vietnamese 404); 0 TS errors
18/07/2026: refactored to 3-step flow — /, /[hostName] (sharing page), /[hostName]/[guestName] (invite card); added displayName to sheet schema and EventConfig; created features/host/HostPage.tsx, features/home/actions.ts (checkHostExists server action), app/[hostName]/[guestName]/page.tsx + loading.tsx; home form validates against sheet before redirect; 0 TS errors
